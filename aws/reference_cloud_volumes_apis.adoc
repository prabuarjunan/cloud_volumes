---
sidebar: sidebar_aws
permalink: aws/reference_cloud_volumes_apis.html
...
==	Finding the URL, API key, and the secret key
You need the URL, API key, and the secret key to run an API call.

.Steps
. Click **API access** on the storage page or in the drop-down menu under your username.
+
. Record the API URL, the API key, and the secret key.

== Listing the available APIs
The storage page displays available APIs that you can use.
...
This section shows you how to use the Cloud Volumes APIs.  The examples use curl from a Linux bash shell. You need to replace `api_url`, `api_key`, and `secret_key` with your values for Cloud Volumes Service.
...
==== Creating a volume
The following example uses a `POST` call to create a volume called `Test`:
``
...
==== Updating a volume
The following example uses a `PUT` call to update a volume called `Test`, change the service level to `extreme`, and change the quota to 500GB:
``
...
``
CAUTION: This API call deletes the volume and all its data.
...
The following example uses a `POST` call to revert a volume from a snapshot by providing `snapshotId` and `fileSystemId`:
``
...
"region": "us-west"}'
``
CAUTION: This API call causes any data written after the snapshot to be lost.
...
``
CAUTION: This API call deletes the snapshot and all its data.
